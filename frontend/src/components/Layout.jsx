...
import { 
  Sun, Moon, Menu, X, LogOut, ChevronRight, ChevronDown, 
  Home, Monitor, Tablet, Smartphone, Bell, Search, User, Globe
} from 'lucide-react';
  ...
  return (
    ...
    >
      <span className={`w-8 h-8 flex items-center justify-center rounded-lg transition-all duration-200 ${
        ...
        {IconComponent ? <IconComponent size={18} /> : <span className="text-base">{item.icon}</span>}
      </span>
...
const NavGroup = memo(({ item, navigate, location, onClick, isExpanded, onToggle }) => {
  const hasActiveChild = item.children?.some(child => location.pathname === child.path);
  const isOpen = isExpanded === true || isExpanded === undefined || hasActiveChild;
    ...
    <div className="space-y-1">
      ...
      >
        <span className={`w-8 h-8 flex items-center justify-center rounded-lg ${
          hasActiveChild ? 'bg-orange-500/30' : 'bg-white/5 group-hover:bg-orange-500/20'
        }`}>
...
export default function Layout({ children }) {
  ...
  return (
    <div className="flex h-screen transition-colors duration-300" style={{ 
      ...
      <aside className={`${showSidebar ? 'hidden md:flex' : 'hidden'} md:flex-col md:w-60 text-white shadow-2xl`} style={{
        background: isDark 
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 truncate max-w-[120px]">{tenant?.name || 'Entreprise'}</p>
            </div>
          </div>
        </button>
        ...
        <div className="p-3 border-t border-white/10 space-y-2">
          ...
          <div className="flex items-center gap-3 px-3 py-2 rounded-xl bg-white/5">
            ...
            <div className="flex-1 min-w-0">
              <p className="font-medium text-sm truncate">{user?.name || 'N/A'}</p>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="backdrop-blur-md border-b border-orange-400/30 px-2 sm:px-4 lg:px-6 py-1.5 sm:py-2 lg:py-2.5 flex justify-between items-center shadow-md transition-all duration-200" style={{ backgroundColor: '#FF9F43' }}>
          {/* Left - Mobile menu + Breadcrumb - Responsive */}
          <div className="flex items-center gap-1.5 sm:gap-2 lg:gap-3">
            {/* Menu burger visible en mode tablette/mobile OU sur petit écran */}
            <button 
              onClick={() => setMobileMenuOpen(true)}
              className={`p-1.5 sm:p-2 rounded-lg bg-white/20 text-white hover:bg-white/30 transition-colors ${showSidebar ? 'md:hidden' : ''}`}
            >
              ...
              <div className="hidden md:flex items-center gap-2 text-sm">
                ...
                <span className="text-white font-medium">
                  {menuItems.find(m => m.path === location.pathname)?.label || 
                   menuItems.flatMap(m => m.children || []).find(c => c.path === location.pathname)?.label ||
                   'Page'}
                </span>
              </div>
            )}
            ...
            <button 
              onClick={() => navigate('/home')}
              className={`flex items-center gap-1.5 sm:gap-2 ${showSidebar ? 'md:hidden' : ''}`}
            >
              <div className="w-6 h-6 sm:w-7 sm:h-7 lg:w-8 lg:h-8 bg-white/20 rounded-md sm:rounded-lg flex items-center justify-center">
                <span className="text-xs sm:text-sm font-black text-white">S</span>
              </div>
              <span className="font-semibold sm:font-bold text-white text-sm sm:text-base">SIGEC</span>
            </button>
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="hidden xl:flex items-center gap-1 p-1.5 bg-white/20 rounded-xl">
              <button 
                ...
                className={`p-2 rounded-lg transition-all ${viewMode === 'desktop' ? 'text-white bg-white/30 shadow-sm' : 'text-white/70 hover:bg-white/20'}`} 
                title="Desktop"
              >
                <Monitor size={18} />
              </button>
              <button 
                ...
                className={`p-2 rounded-lg transition-all ${viewMode === 'tablet' ? 'text-white bg-white/30 shadow-sm' : 'text-white/70 hover:bg-white/20'}`} 
                title="Tablet"
              >
                <Tablet size={18} />
              </button>
              <button 
                ...
                className={`p-2 rounded-lg transition-all ${viewMode === 'mobile' ? 'text-white bg-white/30 shadow-sm' : 'text-white/70 hover:bg-white/20'}`} 
                title="Mobile"
              >
                <Smartphone size={18} />
              </button>
            ...
            >
              ...
              <kbd className="hidden lg:inline px-1.5 py-0.5 text-xs bg-white/20 rounded-md font-mono">⌘K</kbd>
            </button>
            ...
            <div className="flex items-center gap-2 pl-3 ml-2 border-l-2 border-white/30">
              ...
              <span className="hidden lg:block text-white font-medium text-sm max-w-[100px] truncate">{user?.name}</span>
            </div>
          </div>
        </header>
        ...
        <main className="flex-1 overflow-auto p-4">
          <div className={`animate-fade-in transition-all duration-300 ${getContentMaxWidth()}`}>
            {children}
          </div>
        </main>
      </div>
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <div className="absolute left-0 top-0 bottom-0 w-72 shadow-2xl animate-slide-right text-white" style={{
            background: isDark 
              ? 'linear-gradient(180deg, #252540 0%, #1A1A2E 100%)'
              : 'linear-gradient(180deg, #2D3748 0%, #1A202C 100%)'
          }}>
            ...
            {/* Footer */}
            <div className="absolute bottom-0 left-0 right-0 p-3 border-t border-white/10 space-y-2" style={{
