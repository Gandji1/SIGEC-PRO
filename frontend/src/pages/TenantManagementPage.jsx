...
export default function TenantManagementPage() {
  ...
  const [deleteModal, setDeleteModal] = useState({ show: false, tenant: null });
  ...
  const fetchTenants = async () => {
    ...
    try {
      ...
      setError(
        err.response?.data?.message || "Erreur lors du chargement des tenants",
      );
      setTenants([]);
    } finally {
      setLoading(false);
  ...
  const handleCreateTenant = async (e) => {
    ...
    try {
      const payload = {
        ...
        owner_name:
          formData.owner_name.trim() || formData.name.trim() + " Owner",
  ...
  const handleDeleteTenant = async () => {
    ...
    try {
      const res = await apiClient.delete(
        `/superadmin/tenants/${deleteModal.tenant.id}`,
      );
      if (res.data?.success) {
        setSuccess(
          `✅ Tenant "${deleteModal.tenant.name}" supprimé définitivement`,
        );
      ...
      <div className="flex items-center justify-center h-screen">
        <div className="text-center">
          ...
          <p className="text-gray-600">
            Veuillez vous connecter en tant que SuperAdmin
          </p>
          <a
            href="/login"
            ...
            {tenants.map((t) => (
              <tr key={t.id} className="border-b hover:bg-gray-50">
                ...
                <td className="px-6 py-4 space-x-2">
                  ...
                  <button
                    onClick={() => setDeleteModal({ show: true, tenant: t })}
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
            ...
            <div className="p-6">
              ...
              <p className="text-center text-gray-700 mb-2">
                Êtes-vous sûr de vouloir supprimer définitivement le tenant
              </p>
              ...
              <div className="bg-red-50 border border-red-200 rounded-lg p-3 mb-4">
                <p className="text-sm text-red-700 text-center">
                  ...
                  Toutes les données associées (utilisateurs, produits, ventes,
                  etc.) seront supprimées définitivement.
                </p>
              </div>
              <div className="flex gap-3">
                <button
                  ...
                  disabled={deleting}
                  className="flex-1 border border-gray-300 text-gray-700 hover:bg-gray-50 py-2 rounded-lg font-medium transition disabled:opacity-50"
                >
                  Annuler
                </button>
                <button
