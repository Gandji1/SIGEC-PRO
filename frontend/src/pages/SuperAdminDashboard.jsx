...
export default function SuperAdminDashboard() {
  ...
  const systemKpis = [
    ...
    {
      title: "Erreurs 24h",
      value: stats.critical_logs || 0,
      icon: AlertTriangle,
      color: stats.critical_logs > 0 ? "red" : "green",
    },
    {
      title: "Base de données",
      value: `${stats.db_size || 0} MB`,
      icon: Database,
      color: "indigo",
    },
    {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className="bg-orange-500 hover:bg-orange-600 text-white px-3 py-2 rounded-xl font-medium text-sm flex items-center gap-1"
          >
            <Building2 size={16} />
            Nouveau Tenant
          </button>
        ...
        >
          ...
          <div>
            ...
            <span className="text-gray-500 ml-2 text-sm">
              DB: {health.checks?.database?.status || "N/A"} | Cache:{" "}
              {health.checks?.cache?.status || "N/A"} | Disque:{" "}
              {health.checks?.storage?.message || "N/A"}
            </span>
          </div>
        </div>
      )}
          ...
          : kpis.map((kpi, idx) => (
              <div
                ...
                className={`bg-gradient-to-br ${
                  colorMap[kpi.color]
                } dark:from-slate-800 dark:to-slate-800 dark:border-slate-700 border rounded-xl p-4`}
              >
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs font-semibold mb-1 dark:text-gray-300">
                      {kpi.title}
                    </p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <button
          onClick={() => navigate("/tenant-management")}
          className="bg-blue-100 hover:bg-blue-200 text-blue-700 font-medium py-4 px-4 rounded-xl transition flex items-center justify-center gap-2"
        >
          <Building2 size={20} /> Gérer Tenants
        </button>
        <button
          onClick={() => navigate("/subscriptions")}
          className="bg-purple-100 hover:bg-purple-200 text-purple-700 font-medium py-4 px-4 rounded-xl transition flex items-center justify-center gap-2"
        >
          <CreditCard size={20} /> Abonnements
        </button>
        <button
          onClick={() => navigate("/payment-gateways")}
          className="bg-green-100 hover:bg-green-200 text-green-700 font-medium py-4 px-4 rounded-xl transition flex items-center justify-center gap-2"
        >
          <CreditCard size={20} /> Passerelles Paiement
        </button>
        <button
          onClick={() => navigate("/system-logs")}
          className="bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium py-4 px-4 rounded-xl transition flex items-center justify-center gap-2"
        >
          <Server size={20} /> Logs Système
        </button>
        <button
          onClick={() => navigate("/platform-settings")}
          className="bg-orange-100 hover:bg-orange-200 text-orange-700 font-medium py-4 px-4 rounded-xl transition flex items-center justify-center gap-2"
        >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="text-center p-4 bg-gray-50 rounded-lg">
            <p className="text-2xl font-bold text-green-600">
              {stats.api_response_time || "< 100"}ms
            </p>
