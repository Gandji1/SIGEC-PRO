...
export default function ImmobilisationsPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (selectedImmo) {
        await apiClient.put(
          `/accounting/immobilisations/${selectedImmo.id}`,
          form,
        );
      } else {
  ...
  const calculateAmortissements = async () => {
    try {
      const res = await apiClient.post(
        "/accounting/immobilisations/calculate",
        { year: new Date().getFullYear() },
      );
      alert(
        `✅ Amortissements calculés: ${res.data?.total_amortissement?.toLocaleString("fr-FR")} FCFA`,
      );
      fetchImmobilisations();
    } catch (error) {
  ...
  const formatCurrency = (val) =>
    new Intl.NumberFormat("fr-FR").format(val || 0) + " FCFA";

  // Totaux
  const totals = immobilisations.reduce(
    (acc, immo) => ({
      valeur_acquisition:
        acc.valeur_acquisition + parseFloat(immo.valeur_acquisition || 0),
      cumul_amortissement:
        acc.cumul_amortissement + parseFloat(immo.cumul_amortissement || 0),
      vnc: acc.vnc + parseFloat(immo.vnc || 0),
    }),
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Gestion des actifs immobilisés - SYSCOHADA Révisé
          </p>
        </div>
        <button
          onClick={() => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div className="bg-blue-50 rounded-xl p-4 border border-blue-200">
          ...
          <p className="text-xl font-bold text-blue-800">
            {formatCurrency(totals.valeur_acquisition)}
          </p>
        </div>
        <div className="bg-orange-50 rounded-xl p-4 border border-orange-200">
          ...
          <p className="text-xl font-bold text-orange-800">
            {formatCurrency(totals.cumul_amortissement)}
          </p>
        </div>
                ...
                immobilisations.map((immo) => (
                  <tr key={immo.id} className="hover:bg-gray-50">
                    ...
                    <td className="py-3 px-4 text-center text-gray-600">
                      {new Date(immo.date_acquisition).toLocaleDateString(
                        "fr-FR",
                      )}
                    </td>
                    <td className="py-3 px-4 text-right font-medium">
                      {formatCurrency(immo.valeur_acquisition)}
                    </td>
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-4 border-b flex justify-between items-center">
              <h2 className="text-lg font-bold">
                {selectedImmo ? "Modifier" : "Nouvelle"} Immobilisation
              </h2>
              <button
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setForm({ ...form, valeur_residuelle: e.target.value })
                    }
                    className="w-full border rounded-lg px-3 py-2"
                    min="0"
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setForm({ ...form, duree_vie: e.target.value })
                    }
                    className="w-full border rounded-lg px-3 py-2"
                    min="1"
                    max="50"
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setForm({
                        ...form,
                        methode_amortissement: e.target.value,
                      })
                    }
                    className="w-full border rounded-lg px-3 py-2"
                  >
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                <div className="bg-gray-50 p-3 rounded-lg">
                  ...
                  <p className="font-bold">
                    {formatCurrency(selectedImmo.valeur_acquisition)}
                  </p>
                </div>
                <div className="bg-gray-50 p-3 rounded-lg">
                  <p className="text-xs text-gray-500">Valeur résiduelle</p>
                  <p className="font-bold">
                    {formatCurrency(selectedImmo.valeur_residuelle)}
                  </p>
                </div>
                <div className="bg-gray-50 p-3 rounded-lg">
                ...
                <div className="bg-gray-50 p-3 rounded-lg">
                  ...
                  <p className="font-bold capitalize">
                    {selectedImmo.methode_amortissement}
                  </p>
                </div>
              </div>
              ...
              <table className="w-full text-sm border">
                <thead className="bg-gray-100">
                  <tr>
                    ...
                    <th className="py-2 px-3 text-right border">
                      Base amortissable
                    </th>
                  ...
                  {calculateAmortissement(selectedImmo).map((row, idx) => (
                    ...
                    >
                      ...
                      <td className="py-2 px-3 border text-right">
                        {formatCurrency(row.base_amortissable)}
                      </td>
                      <td className="py-2 px-3 border text-right text-orange-600">
                        {formatCurrency(row.annuite)}
                      </td>
                      <td className="py-2 px-3 border text-right">
                        {formatCurrency(row.cumul_amortissement)}
                      </td>
                      <td className="py-2 px-3 border text-right font-bold text-green-600">
                        {formatCurrency(row.vnc)}
                      </td>
                    </tr>
                  ))}
                </tbody>
