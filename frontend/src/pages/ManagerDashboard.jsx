...
// Composant KPI - MOBILE OPTIMIZED
const KPI = memo(({ title, value, icon, color }) => (
  <div className="bg-white dark:bg-gray-800 rounded-lg sm:rounded-xl p-2 sm:p-4 border border-gray-200 dark:border-gray-700 shadow-sm">
    <div className="flex items-center gap-2 sm:gap-3">
      <div className={`${color} w-8 h-8 sm:w-11 sm:h-11 rounded-md sm:rounded-lg flex items-center justify-center text-white shadow flex-shrink-0`}>{icon}</div>
      <div className="flex-1 min-w-0">
        <p className="text-[9px] sm:text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide truncate">{title}</p>
...
export default function ManagerDashboard() {
  ...
  return (
    <div className="p-3 sm:p-4 md:p-6 space-y-3 sm:space-y-5 bg-gray-50 dark:bg-gray-900 min-h-screen">
      {/* Header - MOBILE OPTIMIZED */}
      <div className="flex justify-between items-center gap-2">
        <div className="min-w-0">
          <h1 className="text-lg sm:text-2xl font-bold text-gray-900 dark:text-white truncate">Tableau de Bord</h1>
          <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 truncate">{tenant?.name}</p>
        </div>
        <button
          onClick={fetchAllData}
          className="inline-flex items-center gap-1 sm:gap-2 px-2 sm:px-4 py-1.5 sm:py-2 bg-orange-500 hover:bg-orange-600 text-white rounded-lg sm:rounded-xl text-xs sm:text-sm font-medium transition-colors flex-shrink-0"
        >
          <RefreshCw size={14} />
      ...
      {/* KPIs principaux - MOBILE: 2 cols, DESKTOP: 5 cols */}
      ...
      {/* Graphique + Caisse - MOBILE OPTIMIZED */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 sm:gap-4">
        {/* Graphique Ventes 7 jours */}
        <div className="lg:col-span-2 bg-white dark:bg-gray-800 rounded-lg sm:rounded-xl p-3 sm:p-5 border border-gray-200 dark:border-gray-700 shadow-sm">
          <div className="flex items-center justify-between mb-2 sm:mb-4">
            <h3 className="text-sm sm:text-base font-bold text-gray-800 dark:text-white flex items-center gap-2">
              <BarChart3 size={16} className="text-blue-500" />
              <span className="hidden sm:inline">Ventes des 7 derniers jours</span>
              <span className="sm:hidden">Ventes 7j</span>
            </h3>
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <span className="text-[10px] sm:text-xs font-semibold text-blue-600">{fmtC(data.salesTrend.reduce((s, d) => s + d.ventes, 0))}</span>
            </div>
          </div>
          
          {/* Graphique avec axes - hauteur réduite sur mobile */}
          <div className="relative h-40 sm:h-56 mt-1 sm:mt-2">
        ...
        {/* Caisse du Jour - MOBILE OPTIMIZED */}
        {data.cashData && (
          <div className="bg-gradient-to-br from-emerald-500 to-teal-600 rounded-lg sm:rounded-xl p-3 sm:p-5 text-white shadow-lg">
            <div className="flex items-center justify-between mb-2 sm:mb-4">
              <h3 className="text-sm sm:text-base font-bold flex items-center gap-2">
                <Wallet size={16} /> Caisse
              </h3>
              <Link to="/cash-register" className="bg-white/20 hover:bg-white/30 px-2 py-1 rounded text-[10px] sm:text-xs flex items-center gap-1">
                Détails <ArrowRight size={10} />
              </Link>
            </div>
            <div className="space-y-2 sm:space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-emerald-100 text-xs sm:text-sm">Entrées</span>
                <span className="font-bold text-sm sm:text-base">{fmtC(data.cashData.today?.total_in)}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-emerald-100 text-xs sm:text-sm">Sorties</span>
                <span className="font-bold text-sm sm:text-base">{fmtC(data.cashData.today?.total_out)}</span>
              </div>
              <div className="border-t border-white/20 pt-2 sm:pt-3 flex justify-between items-center">
                <span className="text-emerald-100 text-xs sm:text-sm">Solde</span>
                <span className="text-base sm:text-xl font-bold">{fmtC(data.cashData.today?.balance)}</span>
              </div>
            </div>
          </div>
        )}
      ...
      {/* Top Produits + Stock Critique - MOBILE OPTIMIZED */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-4">
        {/* Top Produits Vendus */}
        <div className="bg-white dark:bg-gray-800 rounded-lg sm:rounded-xl p-3 sm:p-5 border border-gray-200 dark:border-gray-700 shadow-sm">
          <h3 className="text-sm sm:text-base font-bold text-gray-800 dark:text-white mb-2 sm:mb-4 flex items-center gap-2">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg sm:rounded-xl p-3 sm:p-5 border border-gray-200 dark:border-gray-700 shadow-sm">
          <div className="flex items-center justify-between mb-2 sm:mb-4">
            ...
            <Link to="/inventory-enriched" className="text-[10px] sm:text-xs text-orange-600 hover:underline flex items-center gap-1">
      ...
      <div className="grid grid-cols-4 gap-2 sm:gap-3">
        <Link to="/transfers" className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg sm:rounded-xl p-2 sm:p-4 hover:shadow-md transition flex flex-col sm:flex-row items-center gap-1 sm:gap-3">
          <div className="w-8 h-8 sm:w-10 sm:h-10 bg-blue-100 dark:bg-blue-900/30 rounded-md sm:rounded-lg flex items-center justify-center flex-shrink-0">
            <Send size={16} className="text-blue-600" />
          </div>
          <div className="text-center sm:text-left">
            <p className="font-medium text-gray-900 dark:text-white text-[10px] sm:text-sm">Transferts</p>
            <p className="text-[9px] sm:text-xs text-gray-500 hidden sm:block">{data.pendingTransfers} en attente</p>
          </div>
        </Link>
        <Link to="/pos/manager-orders" className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg sm:rounded-xl p-2 sm:p-4 hover:shadow-md transition flex flex-col sm:flex-row items-center gap-1 sm:gap-3">
          <div className="w-8 h-8 sm:w-10 sm:h-10 bg-green-100 dark:bg-green-900/30 rounded-md sm:rounded-lg flex items-center justify-center flex-shrink-0">
            <ShoppingCart size={16} className="text-green-600" />
          </div>
          <div className="text-center sm:text-left">
            <p className="font-medium text-gray-900 dark:text-white text-[10px] sm:text-sm">Commandes</p>
            <p className="text-[9px] sm:text-xs text-gray-500 hidden sm:block">Historique</p>
          </div>
        </Link>
        <Link to="/purchases" className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg sm:rounded-xl p-2 sm:p-4 hover:shadow-md transition flex flex-col sm:flex-row items-center gap-1 sm:gap-3">
          <div className="w-8 h-8 sm:w-10 sm:h-10 bg-purple-100 dark:bg-purple-900/30 rounded-md sm:rounded-lg flex items-center justify-center flex-shrink-0">
            <Package size={16} className="text-purple-600" />
          </div>
          <div className="text-center sm:text-left">
            <p className="font-medium text-gray-900 dark:text-white text-[10px] sm:text-sm">Achats</p>
            <p className="text-[9px] sm:text-xs text-gray-500 hidden sm:block">{data.pendingPurchases} en cours</p>
          </div>
        </Link>
        <Link to="/cash-register" className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg sm:rounded-xl p-2 sm:p-4 hover:shadow-md transition flex flex-col sm:flex-row items-center gap-1 sm:gap-3">
          <div className="w-8 h-8 sm:w-10 sm:h-10 bg-emerald-100 dark:bg-emerald-900/30 rounded-md sm:rounded-lg flex items-center justify-center flex-shrink-0">
            <Wallet size={16} className="text-emerald-600" />
          </div>
          <div className="text-center sm:text-left">
            <p className="font-medium text-gray-900 dark:text-white text-[10px] sm:text-sm">Caisse</p>
            <p className="text-[9px] sm:text-xs text-gray-500 hidden sm:block">Gestion</p>
          </div>
        </Link>
      </div>
    </div>
